...
class RestService(object):
    ...
    def feed(self):
        """
        url = {"url": "http://www.baidu.com", "appid":"dfy", "crawlid":"1",
        "attrs": {"title": "a:attr(title)", "time": "span", "href": "a:attr(title)"}}
        :return:
        """

        if self.kafka_connected:
            # json_item = {u'uuid': u'abc123', u'appid': u'stuff'}
            json_item = request.get_json()
            print json_item
            self.wait_for_response = False

            # # 从request中提取启动进程个数，从redis取每个节点 进程个数，选择最少的前几个，启动爬虫
            process_num = int(json_item.pop('process_num', ''))

            nodes = []
            if self.redis_connected:
                # self.logger.debug("Distribution node for user")
                self.logger.debug("Query node state")
                keys = self.redis_conn.keys('stats:*:info')
                node_dict = {}
                for key in keys:
                    elements = key.split(':')
                    node_ip = elements[1]
                    node_stats = self.redis_conn.lindex(key, 0)

                    if node_stats == 'online':
                        node_process_num = int(self.redis_conn.lindex(key, 1))
                        node_dict[node_ip] = node_process_num

                nodes = sorted(node_dict.iteritems(), key=lambda d: d[1])
                # [('192.168.1.222', 6), ('192.168.1.222', 8), ('192.168.1.222', 8), ('192.168.1.222', 10)]
            else:
                self.logger.warning("Can not query node state")

            project = 'zzh'
            schedule_url = 'http://{ip}:6800/schedule.json'

            # # 中心节点调度
            # while True and process_num:
            #     for node in nodes:
            #         ip = node[0]
            #         spider = json_item['spiderid']
            #         setting = 'appid=' + json_item['appid']
            #         dictdata = {'project': project, 'spider': spider, 'setting': setting}
            #
            #         response = requests.post(schedule_url.format(ip=ip), data=dictdata)
            #         infos = json.loads(response.text)
            #
            #         if infos['status'] == 'ok':
            #             process_num -= 1

            # json_item 是url crawlid  appid  attrs（爬取规则）
            result = self._feed_to_kafka(json_item)

            if 'uuid' in json_item:
                    self.wait_for_response = True
                    with self.uuids_lock:    # 获得锁，处理完 释放
                        self.uuids[json_item['uuid']] = None

            if result:  # 说明request成功送到kafka
                true_response = None
                if self.wait_for_response:   # action  查询状态
