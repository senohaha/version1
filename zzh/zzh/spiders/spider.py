...
from zzh.utils.extractor import FileLinkExtractor
from zzh.utils.readability import Readability
from zzh.scrapy_redis.spiders import RedisSpider
...
class ZzhSpider(RedisSpider):
    ...
    def parse_inform_index(self, response):
        ...
        field_css = response.meta['field_css']
        ...
        try:
            ...
            for sel in selector.css(field_css['from']):  # 遍历列表项
                ...
                for field, val in field_css.items():
                    if field in ['from', 'detail', 'continue']:
                        continue
                ...
                if detail_url != 'http://404.html' and con == 'true':
                    yield SplashRequest(url=detail_url, meta={'item': item, 'field_css': field_css['detail']},
